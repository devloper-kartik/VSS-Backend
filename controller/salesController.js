...
eventEmitter.on("OrderRejected", async ({ salesPerson }) => {
  try {
    ...
    const deviceToken =
      "AAAAgt3qpdw:APA91bEENlGDPJDUGNCCkRBTH56yKE1rOc2n7A4UQQnwTENwUoq0keQ8uuxYa_7TS8I7eP9NsgDeXMZcFkA6nWcumcmntaq30cc_aSQAQ_jMmhouh5PXkKSyRABCT-f5PUvCZD9aSIB3";

    console.log(
      `Notification: Order for salesperson -->> ${sales_id} and  --> ${sales_name} is canceled. Notify Sales Manager.`
    );
    ...
    for (const product of req.body.products) {
      ...
      if (stock_data && stock_data.weight >= weight) {
        ...
        if (stock_data && stock_data.weight >= weight) {
          ...
          console.log(
            `Stock for GP Sheet is insufficient. Checking for GP Coil...`
          );
          ...
          if (stock_data && stock_data.weight >= weight) {
            ...
            return res
              ...
              .json({
                status: 404,
                msg: "order can not be placed due to insufficient stock",
              });
          }
        }
      ...
      if (!stock_data) {
        return res
          .status(404)
          .json({ status: 404, msg: "Stock not available" });
      }
    }
...
exports.get = async (req, res) => {
  ...
  const orderList = await salesorder
    .aggregate([
...
exports.edit = async (req, res) => {
  try {
    ...
    if (findsalesOrder) {
      ...
      const updatesalesOrder = await salesorder.findByIdAndUpdate(
        req.params.id,
        req.body,
        { new: true }
      );
...
exports.allRecords = async (req, res) => {
  try {
    ...
    let orderList = await salesorder
      .find()
